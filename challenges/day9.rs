...
impl From<SnowKg> for Snowball {
    fn from(snow: SnowKg) -> Self {
        let snowballs = (snow.0 / SNOWBALL_WEIGHT_KG).round() as i64;
        Snowball(snowballs)
    }
}
impl From<SnowLb> for Snowball {
    fn from(snow: SnowLb) -> Self {
        let snowballs = (snow.0 / SNOWBALL_WEIGHT_LB).round() as i64;
        Snowball(snowballs)
    }
}
